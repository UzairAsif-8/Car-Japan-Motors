...
import { CONTACT, buildWhatsAppLink, CAR_STATUS } from '../constants';
      ...
      {(isSold || isUpcoming) && (
        <div
          className={cn(
            ...
            isSold ? 'bg-red-50' : 'bg-amber-50'
          )}
        >
          <span
            className={cn(
              'font-display text-lg font-extrabold tracking-[0.15em]',
              isSold ? 'text-red-700' : 'text-amber-700'
            )}
          >
            {isSold ? 'SOLD' : 'COMING SOON'}
          </span>
          <p className="mt-1 text-sm text-ink-500">
            {isSold
              ? 'This vehicle has been sold. Browse our available inventory instead.'
              : 'This vehicle is not yet available for inquiry. Check back soon.'}
          </p>
        </div>
      )}
      ...
      {isAvailable ? (
        ...
        <div className="p-6">
          <p className="text-center text-sm text-ink-500">
            {isSold
              ? 'Inquiries are closed for this vehicle.'
              : 'Inquiries will open when this vehicle becomes available.'}
          </p>
          <Button to="/inventory" variant="outline" fullWidth className="mt-4">
            Browse inventory
          </Button>
        </div>
      )}
